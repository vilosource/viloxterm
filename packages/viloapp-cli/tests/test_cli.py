...
class TestCreateCommand:
    ...
    def test_create_basic_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = runner.invoke(
                cli,
                ["create", "test-plugin", "--template", "basic", "--output-dir", str(temp_path)],
            )
    ...
    def test_create_widget_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = runner.invoke(
                cli,
                ["create", "widget-plugin", "--template", "widget", "--output-dir", str(temp_path)],
            )
    ...
    def test_create_plugin_already_exists(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result1 = runner.invoke(cli, ["create", "test-plugin", "--output-dir", str(temp_path)])
            assert result1.exit_code == 0

            # Try to create the same plugin again
            result2 = runner.invoke(cli, ["create", "test-plugin", "--output-dir", str(temp_path)])
...
"""
            ...
            result = runner.invoke(cli, ["--config", str(config_file), "list", "--format", "json"])
...
class TestPackageCommand:
    ...
    def test_package_nonexistent_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = runner.invoke(cli, ["package", str(nonexistent_path)])
    ...
    def test_package_invalid_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = runner.invoke(cli, ["package", str(invalid_dir)])
...
class TestTestCommand:
    ...
    def test_test_nonexistent_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            nonexistent_path = Path(temp_dir) / "nonexistent"
            result = runner.invoke(cli, ["test", str(nonexistent_path)])
...
class TestDevCommand:
    ...
    def test_dev_nonexistent_plugin(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            nonexistent_path = Path(temp_dir) / "nonexistent"
            result = runner.invoke(cli, ["dev", "--plugin", str(nonexistent_path)])
