...
class WidgetDebugger:
    ...
    def _on_state_change(self, widget: AppWidget, from_state: WidgetState, to_state: WidgetState):
        ...
        if from_state == WidgetState.CREATED and to_state == WidgetState.INITIALIZING:
            ...
            if metrics['init_start']:
                ...
                logger.info(f"Widget {widget_id} initialization took {metrics['init_duration']:.3f}s")
    ...
    def print_all_widgets(self):
        ...
        print(f"\n{'='*80}")
        print(f"{'Widget ID':<20} {'Type':<15} {'State':<15} {'Focus':<8} {'Errors':<8} {'Conns':<8}")
        print(f"{'-'*80}")

        for summary in summaries:
            print(f"{summary['widget_id']:<20} "
                  f"{summary['type']:<15} "
                  f"{summary['state']:<15} "
...
class WidgetInspector:
    ...
    def validate_state(widget: AppWidget) -> list[str]:
        ...
        if widget.widget_state == WidgetState.DESTROYED and widget._signal_manager.has_connections():
        ...
        if widget.has_focus and widget.widget_state not in [WidgetState.READY, WidgetState.SUSPENDED]:
    ...
    def compare_widgets(widget1: AppWidget, widget2: AppWidget) -> dict[str, Any]:
        ...
        return {
            ...
            'focus_diff': {
                'widget1': widget1.has_focus,
                'widget2': widget2.has_focus
            },
            'error_diff': {
...
def enable_widget_debugging():
    ...
    logging.getLogger('ui.widgets').setLevel(logging.DEBUG)
