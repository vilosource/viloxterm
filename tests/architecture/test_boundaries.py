...
class TestArchitectureBoundaries:
    ...
    def test_no_plugin_imports_in_core(self):
        ...
        if violations:
            pytest.fail(f"Core code importing from plugins:\n" + "\n".join(violations))
    ...
    def test_plugins_use_sdk_only(self):
                    ...
                    for i, line in enumerate(lines, 1):
                        ...
                        for forbidden in forbidden_imports:
                            if forbidden in line and not line.startswith('#'):
                                ...
                                if 'from core.plugin_system' not in line and 'import core.plugin_system' not in line:
                                    rel_path = os.path.relpath(file_path, base_path)
        ...
        if violations:
            pytest.fail(f"Plugins importing from core (not allowed):\n" + "\n".join(violations))
    ...
    def test_service_locator_usage(self):
        ...
        if violations:
            print(f"Warning - Direct service instantiation found (consider using ServiceLocator):\n" + "\n".join(violations))
    ...
    def test_plugin_system_isolation(self):
        ...
        for file_path in files:
            ...
            try:
                ...
                if 'from core.plugin_system' in content or 'import core.plugin_system' in content:
        ...
        if violations:
            pytest.fail(f"Unexpected plugin system imports:\n" + "\n".join(violations))
    ...
    def test_command_pattern_compliance(self):
        ...
        if violations:
            print(f"Info - Possible command pattern bypasses found:\n" + "\n".join(violations))
